...
class LinksCommand
  ...
  def run(*args)
    ...
    db = SQLite3::Database.new(db_path)
    rows = db.execute(
      'SELECT l.target_id, l.link_type, n.path, n.title FROM links l ' \
      'LEFT JOIN notes n ON l.target_id = n.id ' \
      'WHERE l.source_id = ? ORDER BY l.link_type, n.title',
      [note_id]
    )
    db.close
    ...
    # Build result data
    results = rows.map do |target_id, link_type, path, title|
      {
        'id' => target_id,
        'path' => path,
  ...
  def output_help
    puts <<~HELP
      ...
      EXAMPLES:
          zh links abc12345
          zh links abc12345 --json
          zh links abc12345 --format path
          zh links "My Note Title" --format full
    HELP
  end
end
